...
//   ACCESS_CODE        - 팀 내부용 접속 암호 (임의 문자열, 예: NE2026)
...
module.exports = async (req, res) => {
  ...
  if (!process.env.ACCESS_CODE) {
    res.status(500).json({ error: 'ACCESS_CODE 환경변수가 설정되지 않았습니다. Vercel 프로젝트 설정을 확인하십시오.' });
    return;
  }
  if (code !== process.env.ACCESS_CODE) {
    res.status(401).json({ error: '접속 암호가 올바르지 않습니다.' });
    return;
  }
